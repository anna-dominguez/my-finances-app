import { PrismaClient } from '@prisma/client';
const prisma = new PrismaClient();

async function main() {
	// Create User first as the models depend on it
	const user = await prisma.user.create({
		data: {
			email: 'ann4.dominguez@gmail.com',
			name: 'admin',
			password: 'admin',
		},
	});

	// Seed the Balance
	await prisma.balance.create({
		data: {
			current: 4836.0,
			income: 3814.25,
			expenses: 1700.5,
			userId: user.id,
		},
	});

	// Seed Transactions
	const transactions = [
		{
			avatar: '/avatars/emma-richardson.jpg',
			name: 'Emma Richardson',
			category: 'General',
			date: new Date('2024-08-19T14:23:11Z'),
			amount: 75.5,
			recurring: false,
		},
		{
			avatar: '/avatars/savory-bites-bistro.jpg',
			name: 'Savory Bites Bistro',
			category: 'Dining Out',
			date: new Date('2024-08-19T20:23:11Z'),
			amount: -55.5,
			recurring: false,
		},
		{
			avatar: '/avatars/daniel-carter.jpg',
			name: 'Daniel Carter',
			category: 'General',
			date: new Date('2024-08-18T09:45:32Z'),
			amount: -42.3,
			recurring: false,
		},
		{
			avatar: '/avatars/sun-park.jpg',
			name: 'Sun Park',
			category: 'General',
			date: new Date('2024-08-17T16:12:05Z'),
			amount: 120.0,
			recurring: false,
		},
		{
			avatar: '/avatars/urban-services-hub.jpg',
			name: 'Urban Services Hub',
			category: 'General',
			date: new Date('2024-08-17T21:08:09Z'),
			amount: -65.0,
			recurring: false,
		},
		{
			avatar: '/avatars/liam-hughes.jpg',
			name: 'Liam Hughes',
			category: 'Groceries',
			date: new Date('2024-08-15T18:20:33Z'),
			amount: 65.75,
			recurring: false,
		},
		{
			avatar: '/avatars/lily-ramirez.jpg',
			name: 'Lily Ramirez',
			category: 'General',
			date: new Date('2024-08-14T13:05:27Z'),
			amount: 50.0,
			recurring: false,
		},
		{
			avatar: '/avatars/ethan-clark.jpg',
			name: 'Ethan Clark',
			category: 'Dining Out',
			date: new Date('2024-08-13T20:15:59Z'),
			amount: -32.5,
			recurring: false,
		},
		{
			avatar: '/avatars/james-thompson.jpg',
			name: 'James Thompson',
			category: 'Entertainment',
			date: new Date('2024-08-11T15:45:38Z'),
			amount: -5.0,
			recurring: false,
		},
		{
			avatar: '/avatars/pixel-playground.jpg',
			name: 'Pixel Playground',
			category: 'Entertainment',
			date: new Date('2024-08-11T18:45:38Z'),
			amount: -10.0,
			recurring: true,
		},
		{
			avatar: '/avatars/ella-phillips.jpg',
			name: 'Ella Phillips',
			category: 'Dining Out',
			date: new Date('2024-08-10T19:22:51Z'),
			amount: -45.0,
			recurring: false,
		},
		{
			avatar: '/avatars/sofia-peterson.jpg',
			name: 'Sofia Peterson',
			category: 'Transportation',
			date: new Date('2024-08-08T08:55:17Z'),
			amount: -15.0,
			recurring: false,
		},
		{
			avatar: '/avatars/mason-martinez.jpg',
			name: 'Mason Martinez',
			category: 'Lifestyle',
			date: new Date('2024-08-07T17:40:29Z'),
			amount: -35.25,
			recurring: false,
		},
		{
			avatar: '/avatars/green-plate-eatery.jpg',
			name: 'Green Plate Eatery',
			category: 'Groceries',
			date: new Date('2024-08-06T08:25:44Z'),
			amount: -78.5,
			recurring: false,
		},
		{
			avatar: '/avatars/sebastian-cook.jpg',
			name: 'Sebastian Cook',
			category: 'Transportation',
			date: new Date('2024-08-06T10:05:44Z'),
			amount: -22.5,
			recurring: false,
		},
		{
			avatar: '/avatars/william-harris.jpg',
			name: 'William Harris',
			category: 'Personal Care',
			date: new Date('2024-08-05T14:30:56Z'),
			amount: -10.0,
			recurring: false,
		},
		{
			avatar: '/avatars/elevate-education.jpg',
			name: 'Elevate Education',
			category: 'Education',
			date: new Date('2024-08-04T11:15:22Z'),
			amount: -50.0,
			recurring: true,
		},
		{
			avatar: '/avatars/serenity-spa-and-wellness.jpg',
			name: 'Serenity Spa & Wellness',
			category: 'Personal Care',
			date: new Date('2024-08-03T14:00:37Z'),
			amount: -30.0,
			recurring: true,
		},
		{
			avatar: '/avatars/spark-electric-solutions.jpg',
			name: 'Spark Electric Solutions',
			category: 'Bills',
			date: new Date('2024-08-02T09:25:11Z'),
			amount: -100.0,
			recurring: true,
		},
		{
			avatar: '/avatars/rina-sato.jpg',
			name: 'Rina Sato',
			category: 'Bills',
			date: new Date('2024-08-02T13:31:11Z'),
			amount: -50.0,
			recurring: false,
		},
		{
			avatar: '/avatars/swift-ride-share.jpg',
			name: 'Swift Ride Share',
			category: 'Transportation',
			date: new Date('2024-08-01T18:40:33Z'),
			amount: -18.75,
			recurring: false,
		},
		{
			avatar: '/avatars/aqua-flow-utilities.jpg',
			name: 'Aqua Flow Utilities',
			category: 'Bills',
			date: new Date('2024-07-30T13:20:14Z'),
			amount: -100.0,
			recurring: true,
		},
		{
			avatar: '/avatars/ecofuel-energy.jpg',
			name: 'EcoFuel Energy',
			category: 'Bills',
			date: new Date('2024-07-29T11:55:29Z'),
			amount: -35.0,
			recurring: true,
		},
		{
			avatar: '/avatars/yuna-kim.jpg',
			name: 'Yuna Kim',
			category: 'Dining Out',
			date: new Date('2024-07-29T13:51:29Z'),
			amount: -28.5,
			recurring: false,
		},
		{
			avatar: '/avatars/flavor-fiesta.jpg',
			name: 'Flavor Fiesta',
			category: 'Dining Out',
			date: new Date('2024-07-27T20:15:06Z'),
			amount: -42.75,
			recurring: false,
		},
		{
			avatar: '/avatars/harper-edwards.jpg',
			name: 'Harper Edwards',
			category: 'Shopping',
			date: new Date('2024-07-26T09:43:23Z'),
			amount: -89.99,
			recurring: false,
		},
		{
			avatar: '/avatars/buzz-marketing-group.jpg',
			name: 'Buzz Marketing Group',
			category: 'General',
			date: new Date('2024-07-26T14:40:23Z'),
			amount: 3358.0,
			recurring: false,
		},
		{
			avatar: '/avatars/technova-innovations.jpg',
			name: 'TechNova Innovations',
			category: 'Shopping',
			date: new Date('2024-07-25T16:25:37Z'),
			amount: -29.99,
			recurring: false,
		},
		{
			avatar: '/avatars/bytewise.jpg',
			name: 'ByteWise',
			category: 'Lifestyle',
			date: new Date('2024-07-23T09:35:14Z'),
			amount: -49.99,
			recurring: true,
		},
		{
			avatar: '/avatars/nimbus-data-storage.jpg',
			name: 'Nimbus Data Storage',
			category: 'Bills',
			date: new Date('2024-07-21T10:05:42Z'),
			amount: -9.99,
			recurring: true,
		},
		{
			avatar: '/avatars/emma-richardson.jpg',
			name: 'Emma Richardson',
			category: 'General',
			date: new Date('2024-07-20T17:30:55Z'),
			amount: -25.0,
			recurring: false,
		},
		{
			avatar: '/avatars/daniel-carter.jpg',
			name: 'Daniel Carter',
			category: 'General',
			date: new Date('2024-07-19T12:45:09Z'),
			amount: 50.0,
			recurring: false,
		},
		{
			avatar: '/avatars/sun-park.jpg',
			name: 'Sun Park',
			category: 'General',
			date: new Date('2024-07-18T19:20:23Z'),
			amount: -38.5,
			recurring: false,
		},
		{
			avatar: '/avatars/harper-edwards.jpg',
			name: 'Harper Edwards',
			category: 'Shopping',
			date: new Date('2024-07-17T14:55:37Z'),
			amount: -29.99,
			recurring: false,
		},
		{
			avatar: '/avatars/liam-hughes.jpg',
			name: 'Liam Hughes',
			category: 'Groceries',
			date: new Date('2024-07-16T10:10:51Z'),
			amount: -52.75,
			recurring: false,
		},
		{
			avatar: '/avatars/lily-ramirez.jpg',
			name: 'Lily Ramirez',
			category: 'General',
			date: new Date('2024-07-15T16:35:04Z'),
			amount: 75.0,
			recurring: false,
		},
		{
			avatar: '/avatars/ethan-clark.jpg',
			name: 'Ethan Clark',
			category: 'Dining Out',
			date: new Date('2024-07-14T20:50:18Z'),
			amount: -41.25,
			recurring: false,
		},
		{
			avatar: '/avatars/rina-sato.jpg',
			name: 'Rina Sato',
			category: 'Entertainment',
			date: new Date('2024-07-13T09:15:32Z'),
			amount: -10.0,
			recurring: false,
		},
		{
			avatar: '/avatars/james-thompson.jpg',
			name: 'James Thompson',
			category: 'Bills',
			date: new Date('2024-07-12T13:40:46Z'),
			amount: -95.5,
			recurring: false,
		},
		{
			avatar: '/avatars/ella-phillips.jpg',
			name: 'Ella Phillips',
			category: 'Dining Out',
			date: new Date('2024-07-11T18:05:59Z'),
			amount: -33.75,
			recurring: false,
		},
		{
			avatar: '/avatars/yuna-kim.jpg',
			name: 'Yuna Kim',
			category: 'Dining Out',
			date: new Date('2024-07-10T12:30:13Z'),
			amount: -27.5,
			recurring: false,
		},
		{
			avatar: '/avatars/sofia-peterson.jpg',
			name: 'Sofia Peterson',
			category: 'Transportation',
			date: new Date('2024-07-09T08:55:27Z'),
			amount: -12.5,
			recurring: false,
		},
		{
			avatar: '/avatars/mason-martinez.jpg',
			name: 'Mason Martinez',
			category: 'Lifestyle',
			date: new Date('2024-07-08T15:20:41Z'),
			amount: -65.0,
			recurring: false,
		},
		{
			avatar: '/avatars/sebastian-cook.jpg',
			name: 'Sebastian Cook',
			category: 'Transportation',
			date: new Date('2024-07-07T11:45:55Z'),
			amount: -20.0,
			recurring: false,
		},
		{
			avatar: '/avatars/william-harris.jpg',
			name: 'William Harris',
			category: 'General',
			date: new Date('2024-07-06T17:10:09Z'),
			amount: 20.0,
			recurring: false,
		},
		{
			avatar: '/avatars/elevate-education.jpg',
			name: 'Elevate Education',
			category: 'Education',
			date: new Date('2024-07-05T11:15:22Z'),
			amount: -50.0,
			recurring: true,
		},
		{
			avatar: '/avatars/serenity-spa-and-wellness.jpg',
			name: 'Serenity Spa & Wellness',
			category: 'Personal Care',
			date: new Date('2024-07-03T14:00:37Z'),
			amount: -30.0,
			recurring: true,
		},
		{
			avatar: '/avatars/spark-electric-solutions.jpg',
			name: 'Spark Electric Solutions',
			category: 'Bills',
			date: new Date('2024-07-02T09:25:51Z'),
			amount: -100.0,
			recurring: true,
		},
		{
			avatar: '/avatars/swift-ride-share.jpg',
			name: 'Swift Ride Share',
			category: 'Transportation',
			date: new Date('2024-07-02T19:50:05Z'),
			amount: -16.5,
			recurring: false,
		},
	];

	for (const transaction of transactions) {
		await prisma.transaction.create({
			data: {
				...transaction,
				userId: user.id,
			},
		});
	}

	// Seed Budgets
	const budgets = [
		{
			category: 'Entertainment',
			maximum: 50.0,
			theme: '#277C78',
		},
		{
			category: 'Bills',
			maximum: 750.0,
			theme: '#82C9D7',
		},
		{
			category: 'Dining Out',
			maximum: 75.0,
			theme: '#F2CDAC',
		},
		{
			category: 'Personal Care',
			maximum: 100.0,
			theme: '#626070',
		},
	];

	for (const budget of budgets) {
		await prisma.budget.create({
			data: {
				...budget,
				userId: user.id,
			},
		});
	}

	// Seed Pots
	const pots = [
		{
			name: 'Savings',
			target: 2000.0,
			total: 159.0,
			theme: '#277C78',
		},
		{
			name: 'Concert Ticket',
			target: 150.0,
			total: 110.0,
			theme: '#626070',
		},
		{
			name: 'Gift',
			target: 150.0,
			total: 110.0,
			theme: '#82C9D7',
		},
		{
			name: 'New Laptop',
			target: 1000.0,
			total: 10.0,
			theme: '#F2CDAC',
		},
		{
			name: 'Holiday',
			target: 1440.0,
			total: 531.0,
			theme: '#826CB0',
		},
	];

	for (const pot of pots) {
		await prisma.pot.create({
			data: {
				...pot,
				userId: user.id,
			},
		});
	}
}

main()
	.catch((e) => {
		console.error(e);
		process.exit(1);
	})
	.finally(async () => {
		await prisma.$disconnect();
	});
